...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for idx, (input, img_name) in enumerate(train_queue):
            ...
            # 自动混合精度上下文管理器
            # 在该上下文内，PyTorch会自动选择合适的精度（FP16/FP32）进行计算
            # 以减少内存占用并提高计算速度
            with autocast():
                # 前向传播计算损失（会自动使用混合精度）
                loss = model._loss(input)

            # 使用scaler缩放损失，防止半精度下溢
            scaler.scale(loss).backward()

            # 梯度裁剪，防止梯度爆炸
            nn.utils.clip_grad_norm_(model.parameters(), 5)

            # scaler.step()会先自动unscale梯度，然后执行optimizer.step()
            # 如果梯度没有溢出，就更新参数；如果溢出，就跳过此次更新
            scaler.step(optimizer)

            # 更新scaler的缩放因子，为下一次迭代做准备
            scaler.update()
