...
def calculate_local_variance(noisy, kernel_size=5):
    """
    使用卷积替代unfold操作计算局部方差，大幅减少内存占用
    Args:
        noisy: 输入张量 (batch, channel, height, width)
        kernel_size: 局部窗口大小
    Returns:
        局部方差张量 (与输入同形状)
    """
    pad = kernel_size // 2
    # 边缘填充（保持输出尺寸与输入一致）
    noisy = nnf.pad(noisy, (pad, pad, pad, pad), mode='reflect')

    # 构建均值卷积核（1x1xC的分组卷积，避免通道间干扰）
    kernel = torch.ones(1, 1, kernel_size, kernel_size, device=noisy.device) / (kernel_size ** 2)
    groups = noisy.size(1)  # 按通道分组卷积

    # 计算局部均值 E[x]
    local_mean = nnf.conv2d(noisy, kernel, groups=groups)

    # 计算局部均值的平方 E[x]^2
    noisy_sq = noisy ** 2
    local_mean_sq = nnf.conv2d(noisy_sq, kernel, groups=groups)
