...
class Network(nn.Module):
    ...
    def forward(self, input):
        ...
        # 增强模块：生成光照图s2
        s2 = checkpoint(self.enhance, L2.detach())  # detach()：L2不参与s2的梯度计算，使用checkpoint
        ...
        # 第二级去噪（处理增强图+光照图的拼接）
        concat1 = torch.cat([H11, s21], 1).detach()
        ...
        # 另一路第二级去噪
        concat2 = torch.cat([H12, s22], 1).detach()
        ...
        # 对原始增强图H2进行第二级去噪
        concat3 = torch.cat([H2, s2], 1).detach()
